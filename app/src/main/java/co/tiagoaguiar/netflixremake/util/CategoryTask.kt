...
class CategoryTask(private val callback: Callback) {
    ...
    fun execute(url: String) {
        ...
        executor.execute {
            ...
            try {
                // nesse momento, estamos utilizando a NOVA-thread [thread paralela] (2)
                val requestURL = URL(url) // abrir uma URL
                urlConnection = requestURL.openConnection() as HttpsURLConnection // abrir a conexão
                urlConnection.readTimeout = 2000 // tempo de leitura (2 segs)
                urlConnection.connectTimeout = 2000 // tempo de conexão do app com o servidor (2 segs)
             ...
             // val jsonAsString = stream.bufferedReader().use { it.readText() } // transformando de bytes - string
